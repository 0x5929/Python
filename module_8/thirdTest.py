...
#            import pdb; pdb.set_trace()
            libc = ctypes.CDLL('libc.dylib', use_errno=True)                                 # reference to standard lib c functions
...
#            free_space = libc.valloc(page_size)                 # reserving freespace in the process memory for the shellcode
	    current_fd = os.open(__file__, os.O_RDWR)
	    free_space = mmap.mmap(current_fd, page_size, flags=mmap.MAP_PRIVATE, prot=mmap.PROT_READ | mmap.PROT_WRITE | mmap.PROT_EXEC)
            if free_space == 0:
                print "could not allocate memory"
	    free_space_ptr = (ctypes.c_char * page_size).from_buffer(free_space)
#            free_space = ctypes.c_void_p(free_space)
#	    res = libc.mprotect(free_space_ptr, page_size,  mmap.PROT_READ | mmap.PROT_WRITE | mmap.PROT_EXEC)
            #if 0 != res:
	    #    print "failed to set protect for the free space", str(res)                            # giving the freespace RWX permissions with code 7
	#	errno = ctypes.get_errno()
	#	print os.strerror(errno)
#		sys.exit(1)
            ctypes.memmove(free_space_ptr, sc, page_size)                            # copying from shellcode pointer to free_space pointer
            run = ctypes.cast(free_space_ptr, ctypes.CFUNCTYPE(ctypes.c_void_p))# cast function that returns void pointer to the alloc free space
            run()                                                           # running the casted function which is the free space containing
	    del run
	    free_space_ptr.flush()
 	    os.close(current_fd)
	    print 'exiting'
            sys.exit()                                                                     # shellcode
...
# should try ctypes.c_int if it complainsl, because from_buffer returns an int address
